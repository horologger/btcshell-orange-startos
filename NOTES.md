...
docker buildx build --platform linux/amd64 --tag horologger/btcshell:v0.0.1 --load .

docker buildx build --platform linux/arm64 --tag horologger/btcshell-orange-startos:v0.0.1 --load .
...
```sh
su - alunde
docker pull horologger/btcshell:v0.0.1
...
-e DATABASE_URI="/data/btcshell.db" \
-e COOKIE_SECRET="666" \
-v /home/alunde/btcshell/data:/data \
-v /t4/lnd:/lnd:ro \
-p 8080:8080 \
--name nwc \
-it horologger/btcshell:v0.0.1 
